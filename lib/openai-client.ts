import OpenAI from 'openai'

// Lazy-initialized OpenAI client (avoids build-time errors when env var is not available)
let _openai: OpenAI | null = null

export function getOpenAIClient(): OpenAI {
  if (!_openai) {
    _openai = new OpenAI({
      apiKey: process.env.OPENAI_API_KEY || '',
    })
  }
  return _openai
}

// Backwards-compatible export (getter triggers lazy init at runtime, not build time)
export const openai = new Proxy({} as OpenAI, {
  get(_target, prop) {
    return (getOpenAIClient() as any)[prop]
  },
})

// Retry configuration
const MAX_RETRIES = 5
const INITIAL_DELAY = 1000 // 1 second

// Network errors that should trigger retry
const RETRYABLE_NETWORK_ERRORS = [
  'ECONNRESET',
  'ETIMEDOUT',
  'ECONNREFUSED',
  'ENOTFOUND',
  'ENETUNREACH',
  'EAI_AGAIN',
  'EPIPE',
  'EHOSTUNREACH',
]

/**
 * Check if error is retryable (rate limit OR network error)
 */
function isRetryableError(error: any): boolean {
  // Rate limit errors
  if (error?.status === 429 || error?.code === 'rate_limit_exceeded') {
    return true
  }
  
  // Network errors (ECONNRESET, ETIMEDOUT, etc.)
  if (error?.code && RETRYABLE_NETWORK_ERRORS.includes(error.code)) {
    return true
  }
  
  // FetchError with network error code
  if (error?.errno && RETRYABLE_NETWORK_ERRORS.includes(error.errno)) {
    return true
  }
  
  // Check error message for network-related issues
  const errorMessage = error?.message?.toLowerCase() || ''
  if (
    errorMessage.includes('econnreset') ||
    errorMessage.includes('etimedout') ||
    errorMessage.includes('socket hang up') ||
    errorMessage.includes('network') ||
    errorMessage.includes('fetch failed')
  ) {
    return true
  }
  
  // OpenAI API 5xx errors (server errors)
  if (error?.status >= 500 && error?.status < 600) {
    return true
  }
  
  return false
}

/**
 * Exponential backoff retry wrapper
 * Retries on: rate limit (429), network errors (ECONNRESET, etc.), and server errors (5xx)
 */
export async function retryWithBackoff<T>(
  fn: () => Promise<T>,
  retries: number = MAX_RETRIES
): Promise<T> {
  for (let i = 0; i < retries; i++) {
    try {
      return await fn()
    } catch (error: any) {
      const isLastRetry = i === retries - 1
      const shouldRetry = isRetryableError(error)
      
      if (isLastRetry || !shouldRetry) {
        throw error
      }

      const delay = INITIAL_DELAY * Math.pow(2, i)
      console.log(`[v0] Retry ${i + 1}/${retries} after ${delay}ms due to: ${error?.code || error?.message || 'unknown error'}`)
      await new Promise(resolve => setTimeout(resolve, delay))
    }
  }
  
  throw new Error('Max retries exceeded')
}
