...
import { checkFSVPCompliance, requiresFSVP, assessSAHCODHARisk, formatFSVPViolationsForReport } from '@/lib/fsvp-validator'
import type { Citation, NutritionFact, TextElement, FSVPSupplier, FSVPImporterProfile } from '@/lib/types'
    ...
    if (isManualEntry) {
      ...
      try {
        ...
        visionResult = {
          ...
          // FIXED: Merge ingredients from ALL sources including nutrition label (back panel often has ingredient list)
          // Priority: dedicated ingredients image > INCI > nutrition panel > PDP
          ingredients: (ingredientsData.ingredients?.length > 0 ? ingredientsData.ingredients 
                       : (inciData.ingredients?.length > 0 ? inciData.ingredients 
                       : (nutritionData.ingredients?.length > 0 ? nutritionData.ingredients 
                       : (pdpData.ingredients || [])))),
          // FIXED: Merge allergens from ALL sources - nutrition label often contains "CONTAINS:" statement
          allergens: [...new Set([
                       ...(ingredientsData.allergens || []),
...
// country_of_origin used for Import Alert country-scope filtering
  ...
  const isImporter = manufacturerInfo.is_importer === true
  console.log('[v0] Import Alert context params — company:', explicitCompanyName || 'auto-detect', '| country:', countryOfOrigin || 'not specified')
  console.log('[v0] FSVP check — is_importer:', isImporter, '| country:', countryOfOrigin)
  ...
  Reason: ${ia.reason_for_alert.slice(0, 180)} | Match: ${ia.match_method}`
    ...
    // ── FSVP COMPLIANCE CHECK (21 CFR Part 1, Subpart L) ────────────────────
    // Check Foreign Supplier Verification Program requirements for importers
    // This check runs ONLY when is_importer = true and country != USA
    let fsvpComplianceResult = null
    if (isImporter && countryOfOrigin && countryOfOrigin.toLowerCase() !== 'usa' && countryOfOrigin.toLowerCase() !== 'united states') {
      console.log('[v0] FSVP: Importer detected with foreign country of origin. Running FSVP compliance check...')
      
      // Fetch user profile for FSVP importer info
      const { data: userProfile } = await supabase
        .from('user_profiles')
        .select('importer_duns, importer_duns_verified, fsvp_qualified_individual, fsvp_qi_title, fsvp_qi_credentials, fsvp_program_active')
        .eq('id', user.id)
        .maybeSingle()
      
      // Fetch linked FSVP supplier if available
      let linkedSupplier: FSVPSupplier | null = null
      if (report.fsvp_supplier_id) {
        const { data: supplierData } = await supabase
          .from('fsvp_suppliers')
          .select('*')
          .eq('id', report.fsvp_supplier_id)
          .eq('importer_user_id', user.id)
          .maybeSingle()
        linkedSupplier = supplierData as FSVPSupplier | null
      }
      
      // Build importer profile for FSVP check
      const importerProfile: FSVPImporterProfile = {
        importer_duns: userProfile?.importer_duns,
        importer_duns_verified: userProfile?.importer_duns_verified || false,
        fsvp_qualified_individual: userProfile?.fsvp_qualified_individual,
        fsvp_qi_title: userProfile?.fsvp_qi_title,
        fsvp_qi_credentials: userProfile?.fsvp_qi_credentials || {},
        fsvp_program_active: userProfile?.fsvp_program_active || false
      }
      
      // Run FSVP compliance check
      fsvpComplianceResult = checkFSVPCompliance(report, importerProfile, linkedSupplier)
      
      console.log('[v0] FSVP Result — Applicable:', fsvpComplianceResult.isApplicable, '| Compliant:', fsvpComplianceResult.isCompliant)
      console.log('[v0] FSVP Violations:', fsvpComplianceResult.violations.length, '| Warnings:', fsvpComplianceResult.warnings.length)
      
      if (fsvpComplianceResult.sahcodhaAssessment?.isHighRisk) {
        console.log('[v0] FSVP SAHCODHA: HIGH RISK product detected!')
        console.log('[v0] FSVP SAHCODHA Hazards:', fsvpComplianceResult.sahcodhaAssessment.hazards.join(', '))
        console.log('[v0] FSVP Required Verification:', fsvpComplianceResult.sahcodhaAssessment.requiredVerification)
      }
      
      // Add FSVP violations to main violations array
      if (fsvpComplianceResult.isApplicable && !fsvpComplianceResult.isCompliant) {
        const fsvpViolationsForReport = formatFSVPViolationsForReport(fsvpComplianceResult)
        for (const fsvpViol of fsvpViolationsForReport) {
          violations.push({
            ...fsvpViol,
            source_type: 'fsvp_compliance'
          })
        }
        console.log('[v0] Added', fsvpViolationsForReport.length, 'FSVP violations to report')
      }
      
      // Add SAHCODHA warning even if otherwise compliant
      if (fsvpComplianceResult.sahcodhaAssessment?.isHighRisk && fsvpComplianceResult.isCompliant) {
        violations.push({
          category: 'FSVP SAHCODHA Risk',
          severity: 'warning' as const,
          description: `This product is classified as SAHCODHA (Serious Adverse Health Consequences or Death) category. ${fsvpComplianceResult.sahcodhaAssessment.rationale}`,
          regulation_reference: '21 CFR 1.506(d)',
          suggested_fix: 'Ensure annual onsite audits of your foreign supplier are scheduled and documented. SAHCODHA products require enhanced verification activities.',
          citations: [{
            regulation_id: '21cfr1',
            section: '1.506(d)',
            text: 'SAHCODHA products require annual onsite audits of foreign supplier facilities.',
  ...
  health_claims: visionResult.detectedClaims || [],
        ...
        // FSVP Compliance fields (21 CFR Part 1, Subpart L)
        fsvp_applicable: fsvpComplianceResult?.isApplicable || false,
        fsvp_compliance_status: fsvpComplianceResult 
          ? (fsvpComplianceResult.isCompliant ? 'compliant' : 'non_compliant')
          : 'not_applicable',
        fsvp_violations: fsvpComplianceResult 
          ? [...fsvpComplianceResult.violations, ...fsvpComplianceResult.warnings]
          : [],
