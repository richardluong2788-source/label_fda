...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, password, turnstileToken, accountType } = body
    
    // Validate account type
    const validAccountTypes = ['importer', 'supplier', 'qi']
    const normalizedAccountType = validAccountTypes.includes(accountType) ? accountType : 'importer'
    ...
    // If signup successful, create/update user_profiles with account_type
