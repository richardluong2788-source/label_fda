...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get records that need re-embedding (NULL embeddings)
    ...
    return NextResponse.json({
      ...
      ...results,
      remaining: remaining,
      hint: remaining && remaining > 0 ? 'Call this endpoint again to process more' : 'All done!'
    })
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      withEmbedding,
      withoutEmbedding,
      progress: total ? `${((withEmbedding || 0) / total * 100).toFixed(1)}%` : '0%'
    })
