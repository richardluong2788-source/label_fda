...
import {
  ...
  Menu, ChevronDown, Shield, Ship, Leaf, FileText,
} from 'lucide-react'
...
export function AppHeader({ email, isAdmin, showAuth = true }: AppHeaderProps) {
  ...
  const navItems = [
    ...
    { href: '/dashboard/ingredient-check', label: 'Ingredient Check',     icon: Leaf,       exact: false },
    { href: '/dashboard/prior-notice',     label: 'Prior Notice',         icon: FileText,   exact: false },
    { href: '/history',                    label: t.nav.history,          icon: History,    exact: false },
    { href: '/dashboard/fsvp',             label: t.nav.fsvp,             icon: Ship,       exact: false },
