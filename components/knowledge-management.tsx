          ...
          <Card className="p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                <Badge variant={updateResults.needs_update > 0 ? 'destructive' : 'default'}>
                  {updateResults.needs_update} can cap nhat
                </Badge>
                <Badge variant="outline">
                  {updateResults.up_to_date} da moi nhat
                </Badge>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {updateResults.parts?.map((p: any) => (
                ...
                >
                  ...
                  {p.needsUpdate && p.dbChunkCount > 0 && (
                    <Button
                      variant="outline"
                      size="sm"
            ...
            {updateResults.needs_update > 0 && (
              <div className="mt-4 p-3 bg-muted rounded-lg">
                <p className="text-sm font-medium mb-1">De cap nhat, chay lenh sau tren may local:</p>
                <code className="text-xs bg-background p-2 rounded block font-mono">
                  node scripts/fetch-ecfr-to-json.mjs{' '}
                  {updateResults.parts
                    ?.filter((p: any) => p.needsUpdate)
                    .map((p: any) => p.part)
                    .join(' ')}
                </code>
              </div>
